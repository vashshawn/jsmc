...
var Game = module.exports = function Game(config) {
    ...
    this.spawn = {};
    this.spawn.x = config.spawnx;
    this.spawn.y = config.spawny;
    this.spawn.z = config.spawnz;
    this.version = Git.HEAD(__dirname + '/../').shortCommit;
    this.world = {
	type: "flat",
	dimension: 0
    };
...
Game.prototype.remove_client = function remove_client(client) {
	...
	console.warn('[ERROR] Failed to remove client: ' + e);
    }
};
...
Game.prototype.remove_player = function remove_player(player) {
    console.log('[PLAYER] Removing player ' + player.name);
