...
Map.prototype.save_blocks = function save_blocks(location, callback) {
    
    async.forEach(Object.keys(this.chunks), function(k, cb) {
	
	if (!this.chunks[k].dirty) { cb(); return; }
	
	fs.writeFile(path.join(location, Buffer(k).toString("base64")), this.chunks[k].data, function(err) {
	    if (err) {
		console.warn(err);
		return;
	    }

	    this.chunks[k].dirty = false;
	    cb();
	}.bind(this));
